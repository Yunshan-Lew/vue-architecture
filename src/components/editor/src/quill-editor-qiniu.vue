<template>
	<div id="quillEditorQiniu">
		...
		<quill-editor class="marb15" v-model="content" ref="customQuillEditor" :options="editorOption"></quill-editor>
	</div>
</template>
	...
	export default {
		...
		methods:{
			...
			uploadEditorSuccess(res, file) {
				...
				//重置上传文件name，为下次上传做好准备
				this.qiniuForm.name = this.qiniuForm.key = "pc_answer_image_xjl_" + new Date().getTime() + Math.floor(Math.random() * 1000);
